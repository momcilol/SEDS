...
address_read_randoms(address_count, 250)
        


with open('Sifarnici/atm.csv', 'w', newline='', encoding='utf-8') as output_file:
    writer = csv.writer(output_file)

    writer.writerow(['IDBANKOMAT','NAZIV','ADRESA','NASELJE_IDNASELJE'])

    
    for i in range(200):
        writer.writerow([i,f'ATM{addresses[i][1]}{i}',addresses[i][0],addresses[i][1]])
        i += 1
        if i % 25 == 0:
            print(i)
